...
uses
  ...
  SynEdit, ClipBrd, system.IOUtils;
...
begin
  ...
  ed.Lines.Delete(ed.Lines.count);
end;
...
procedure TForm30.Clipboard1Click(Sender: TObject);
var sl:tstringlist; i:integer;  str:string;
begin
  sl:=tstringlist.Create;
  try
...
procedure TForm30.Copyfiles1Click(Sender: TObject);
var sl:tstringlist; i:integer;  str,source,target:string;
begin
  source:='C:\ProgramData\BrailleStudio\Resources\natural\';
  target:='c:\000\waves\';
sl:=tstringlist.Create;
  try
    for i:=0 to lst.lines.Count-1 do
    begin
      ...
      if fileexists(source+lst.lines[i]+'.mp3') then
      begin
       sl.Add(lst.Lines[i]);
       if not fileexists(target+lst.Lines[i]+'.mp3') then

      TFile.Copy(source+lst.lines[i]+'.mp3', target+lst.Lines[i]+'.mp3');
      end;
    end;
